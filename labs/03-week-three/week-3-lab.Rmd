...
rootdir <- ("~/Documents/MEDS/fall_term/EDS222/EDS-222-stats")
...
*Both graphs seem positively corraliated,, however the tail length versus head length seems stronger*
...
1. Use the plots from section 1 to write your best estimate of the correlation coefficient between each of the two examples in section 1. 
  (i) the length of a possum's total length and the length of possum tail: +0.70
...
* The correlation between the tail length and the total length is `r cor_total` and the correlation between the tail length and the head length is `r cor_head`


```{r}
cor_total <- cor(x = possum$tail_l, y = possum$total_l) #0.566

cor_head <- cor(x = possum$tail_l, y = possum$head_l) #0.287
```
...
*possums start with a total length of 41.04cm and with every increase of one cm for the tail length the total length increases by 1.24 cm* 

3. Is your estimate of $\beta_0$ directly interpretable? What might you be concerned about when interpreting $\beta_0$? 
*We need to be careful about interpreting $\beta_0$ becasue it is saying that when possums have 0 tails they will have a body length of 40.04 cm. which we know about possums is false. Our data only shows x values from 30 on, so using $\beta_0$ would be extrapolating too much.*
...
*The fact that the increase is over 1 tell us that there is an influence of the body on the total length. becasue if it was just tail to tail it would be a one to one ratio.*
...
```{r}
model_p <- lm(total_l ~ tail_l, data = possum)

added_pred <- add_predictions(data = possum, model = model_p) %>% 
  mutate(residual = total_l - pred)

ggplot(data = added_pred) + 
  geom_histogram(aes(residual))
```


2. Make a histogram of your residuals. Do they look approximately normally distributed?
*it looks relatively normally distributed however, 
