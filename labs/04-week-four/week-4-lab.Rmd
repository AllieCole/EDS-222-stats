...
rootdir <- ("~/Documents/MEDS/fall_term/EDS222/EDS-222-stats/labs")
...
```{r}
mod1 <- lm(total_l ~ tail_l, data = possum) #models from last week
mod2 <- lm(head_l ~ tail_l, data = possum)

r2_total <- summary(mod1)$r.squared
r2_head <- summary(mod2)$r.squared

#getting the r2 value, meaning that 32% of the data variation is explained. regress y on x, dependent on independent. y is what you are trying to explain and x is how you are explaining it. using teh second to explain the first. R is the covariance between two variables divided by . which normilizes the covariance. R2ed is the % of the variance explained by the model. 
...
```{r}
head(mpg) # year is an interger! not great 

mpg <- mpg %>% mutate(year = as.factor(year)) #turn year into a factor so that it doesnt treat it as numbers 

#scatter plot is not helpful with categorical data so we will use a boxplot, it shows us the distribution of the data better. 
...
# what did we learn?
    #that teh medians are pretty similar, but the median went down but two of the quartiles went up across the years so its a bit hard to tell if the fuel efficiency went up or down. 
```
...
*The intercept tells us that for the 1999 car vintage the average fuel efficiency is ~23* 

3. What does the coefficient estimate on the 2008 vintage indicator variable tell you, in words? 
*The coefficient estimate tells us that teh 2008 vintage is 0.025 miles per gallon higher than the 1999 vintage* 

4. Does your model suggest anything about whether fuel efficiency has evolved over time? Why or why not? 
*This model shows that there might be a little bit of an increase in fuel efficiency, however it is very small and may be wrong as it only takes into account one variable. This model does not account for any of the other changes to the cars, or to the buying patterns of people*
...
```{r}
ggplot(data = mpg, aes(displ, y = hwy)) + 
  geom_point()
```
...
```{r}
mod4 <- lm(hwy ~ year + displ, data = mpg) %>% 
  summary() %>% 
  xtable() %>% 
  kable()
...
*intercept is the mpg of a 1999 vintage with a zero displacement sized engine. the displ variable is how much fuel efficency decreases as the displacemnt or size, increases, i one liter size increase will decrease mph by ~3.6 on average for all cars. And then the 2008 variable is showing that if displacemnt is held constant the fuel efficency will increase on average by 1.4. this shows that more poeple might be buying cars with larger engines and larger cars *
...
ggplot(data = mpg, aes(x = displ, y = hwy, color = year)) +
  geom_point() +
  geom_line( data = augment(mod4), aes(y = .fitted, color = year)) + 
